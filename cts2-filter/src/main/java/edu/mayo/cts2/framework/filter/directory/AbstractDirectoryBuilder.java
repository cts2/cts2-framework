...
public abstract class AbstractDirectoryBuilder<T> implements DirectoryBuilder<T> {
	 ...
	 * @param referenceToMatch the name or uri
	 * @param references the references
	 * @return the model attribute reference
	 */
	protected <M extends ComponentReference> M getComponentReference(
			ComponentReference referenceToMatch,
			Iterable<? extends M> references){
		for(M componentReference : references){
            if(ModelUtils.matches(referenceToMatch, componentReference)){
                return componentReference;
            }
		}
		
		throw ExceptionFactory.createUnsupportedComponentReference(
                referenceToMatch,
                references);
	}
